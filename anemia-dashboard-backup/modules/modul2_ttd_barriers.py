...
def generate_wordcloud(tokens_series: pd.Series, color: str) -> Optional[Image.Image]:
    ...
    wordcloud = WordCloud(
        width=2400,  # High resolution for export
        height=1800,
...
def render(dataframe: pd.DataFrame) -> None:
    ...
    # Create main barrier distribution chart
    barrier_chart = barrier_distribution_chart(filtered_counts)
    st.plotly_chart(barrier_chart, use_container_width=True)
    
    # Initialize export artifacts
    export_artifacts = {
        "barrier_distribution": {"type": "plotly", "object": barrier_chart},
    }
    ...
    with tabs[0]:
        ...
        for idx, (category, color) in enumerate(COLOR_MAP.items()):
            ...
            if image:
                ...
                # Store wordcloud for export
                export_artifacts[f"wordcloud_{category.lower().replace(' ', '_')}"] = {
                    "type": "wordcloud",
                    "object": image
                }
            else:
    ...
    with tabs[1]:
        age_chart = breakdown_chart(filtered_df, "Kelompok_Usia", "Sebaran Kendala per Kelompok Usia")
        st.plotly_chart(age_chart, use_container_width=True)
        export_artifacts["breakdown_by_age"] = {"type": "plotly", "object": age_chart}
        
        trimester_breakdown = breakdown_chart(filtered_df, "Trimester", "Sebaran Kendala per Trimester")
        st.plotly_chart(trimester_breakdown, use_container_width=True)
        export_artifacts["breakdown_by_trimester"] = {"type": "plotly", "object": trimester_breakdown}
        
        education_breakdown = breakdown_chart(filtered_df, "Pendidikan", "Sebaran Kendala per Pendidikan")
        st.plotly_chart(education_breakdown, use_container_width=True)
        export_artifacts["breakdown_by_education"] = {"type": "plotly", "object": education_breakdown}
    ...
    with tabs[2]:
        ...
        export_artifacts["sentiment_analysis"] = {"type": "plotly", "object": sentiment_fig}
    ...
    with tabs[3]:
        ...
        export_artifacts["cooccurrence_matrix"] = {
            "type": "dataframe",
            "object": analysis.cooccurrence_matrix,
            "title": "Matriks Ko-Occurence Token"
        }
    ...
    with tabs[4]:
        ...
        if trend_fig.data:
            ...
            export_artifacts["temporal_trend"] = {"type": "plotly", "object": trend_fig}
        else:
            st.info("Data timestamp tidak tersedia untuk analisis tren.")

    with tabs[5]:
        heatmap_fig = geographic_heatmap(filtered_df)
        st.plotly_chart(heatmap_fig, use_container_width=True)
        export_artifacts["geographic_heatmap"] = {"type": "plotly", "object": heatmap_fig}

    st.subheader("Daftar Respons dan Kategori")
    response_list = filtered_df[
        [
            "Nama",
            "Kecamatan",
            "Desa",
    ...
    st.dataframe(response_list, use_container_width=True)
    
    export_artifacts["response_list"] = {
        "type": "dataframe",
        "object": response_list.head(50),  # Limit to 50 rows for export
        "title": "Daftar Respons dan Kategori"
    }
    
    # Store artifacts in session state for export
    st.session_state["module_2_export_artifacts"] = export_artifacts
