...
def render(dataframe: pd.DataFrame) -> None:
    ...
    with tabs[0]:
        ...
        confusion_chart = build_confusion_matrix_chart(
            artifacts.confusion_matrix,
            artifacts.label_encoder.classes_,
        )
        st.plotly_chart(confusion_chart, use_container_width=True)
        export_artifacts["confusion_matrix"] = {"type": "plotly", "object": confusion_chart}

        importance_chart = build_feature_importance_chart(artifacts.feature_importances)
        st.plotly_chart(importance_chart, use_container_width=True)
        export_artifacts["feature_importance"] = {"type": "plotly", "object": importance_chart}
        
        roc_chart = build_roc_curve_chart(artifacts.roc_data)
        st.plotly_chart(roc_chart, use_container_width=True)
        export_artifacts["roc_curve"] = {"type": "plotly", "object": roc_chart}

        st.subheader("Classification Report")
        st.dataframe(artifacts.classification_report, use_container_width=True)
        export_artifacts["classification_report"] = {
            "type": "dataframe",
            "object": artifacts.classification_report,
    ...
    with tabs[1]:
        ...
        export_artifacts["risk_proportion"] = {"type": "plotly", "object": risk_pie_chart}
        ...
        st.plotly_chart(risk_by_age_chart, use_container_width=True)
        export_artifacts["risk_by_age"] = {"type": "plotly", "object": risk_by_age_chart}

        trimester_group = predictions.groupby(["Trimester", "Predicted_Risk"]).size().reset_index(name="Jumlah")
        
        risk_by_trimester_chart = px.bar(
            trimester_group,
            x="Trimester",
            y="Jumlah",
            color="Predicted_Risk",
        ...
        st.plotly_chart(risk_by_trimester_chart, use_container_width=True)
        export_artifacts["risk_by_trimester"] = {"type": "plotly", "object": risk_by_trimester_chart}

        risk_by_kecamatan_chart = px.density_heatmap(
            predictions,
            x="Kecamatan",
            y="Predicted_Risk",
            histfunc="count",
        ...
        st.plotly_chart(risk_by_kecamatan_chart, use_container_width=True)
        export_artifacts["risk_by_kecamatan"] = {"type": "plotly", "object": risk_by_kecamatan_chart}
    ...
    with tabs[4]:
        ...
        if high_risk.empty:
            ...
            export_artifacts["high_risk_list"] = {
                "type": "dataframe",
                "object": high_risk_display.head(50),  # Limit to 50 rows for export
                "title": "Daftar Risiko Tinggi"
            }
    ...
    st.session_state["module_3_export_artifacts"] = export_artifacts
