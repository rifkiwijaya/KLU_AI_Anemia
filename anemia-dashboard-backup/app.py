...
def render_home(dataframe: Optional[pd.DataFrame]) -> None:
    ...
    # Create charts and store in session state for export
    pie_chart = create_anemia_pie_chart(dataframe)
    age_chart = build_anemia_by_age_chart(dataframe)
    trimester_chart = build_anemia_by_trimester_chart(dataframe)
    ttd_chart = build_ttd_vs_anemia_chart(dataframe)
    
    # Store charts for export
    st.session_state["home_export_artifacts"] = {
        "anemia_distribution_pie": {"type": "plotly", "object": pie_chart},
        "anemia_by_age": {"type": "plotly", "object": age_chart},
        "anemia_by_trimester": {"type": "plotly", "object": trimester_chart},
        "ttd_compliance_by_status": {"type": "plotly", "object": ttd_chart},
    }
    
    # Display charts
    chart_cols = st.columns(2)
    chart_cols[0].plotly_chart(pie_chart, use_container_width=True)
    chart_cols[1].plotly_chart(age_chart, use_container_width=True)

    chart_cols = st.columns(2)
    chart_cols[0].plotly_chart(trimester_chart, use_container_width=True)
    chart_cols[1].plotly_chart(ttd_chart, use_container_width=True)
...
def export_current_page() -> Optional[bytes]:
    """Export visualizations from the current page.
    
    Returns:
        ZIP file bytes containing exported images, or None if no artifacts available.
    """
    current_page = st.session_state.get("navigation", "")
    page_key_map = {
        "🏠 Dashboard Utama": "home",
        "📍 Modul 1: Peta Hotspot Anemia": "module_1",
        "📋 Modul 2: Analisis Kendala TTD": "module_2",
    ...
    page_key = page_key_map.get(current_page)
    if not page_key:
        return None
    
    artifact_key = f"{page_key}_export_artifacts"
    
    if artifact_key not in st.session_state:
        st.warning(f"Tidak ada visualisasi tersedia untuk di-export dari halaman ini. Pastikan halaman sudah di-load dengan lengkap.")
        return None
    
    try:
        exports = export_manager.export_session_state_artifacts(st.session_state, page_key)
        if not exports:
            st.warning("Tidak ada visualisasi yang dapat di-export.")
            return None
        
        # Create ZIP with current page exports
        zip_bytes = export_manager.create_export_zip({page_key: exports}, include_composites=True)
        return zip_bytes
    
    except Exception as exc:
        st.error(f"Gagal melakukan export: {exc}")
        logging.exception("Export failed: %s", exc)
        return None


def export_all_pages() -> Optional[bytes]:
    """Export visualizations from all pages.
    
    Returns:
        ZIP file bytes containing all exported images, or None if no artifacts available.
    """
    all_exports = {}
    page_keys = ["home", "module_1", "module_2", "module_3"]
    
    for page_key in page_keys:
        artifact_key = f"{page_key}_export_artifacts"
        if artifact_key in st.session_state:
            try:
                exports = export_manager.export_session_state_artifacts(st.session_state, page_key)
                if exports:
                    all_exports[page_key] = exports
            except Exception as exc:
                logging.error("Failed to export page %s: %s", page_key, exc)
    
    if not all_exports:
        st.warning("Tidak ada visualisasi tersedia untuk di-export. Buka setiap halaman terlebih dahulu untuk me-load visualisasi.")
        return None
    
    try:
        zip_bytes = export_manager.create_export_zip(all_exports, include_composites=True)
        return zip_bytes
    except Exception as exc:
        st.error(f"Gagal membuat ZIP file: {exc}")
        logging.exception("ZIP creation failed: %s", exc)
        return None
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.caption("Export semua chart dan tabel sebagai PNG untuk paper (300 DPI)")
        
        col1, col2 = st.columns(2)
        
        with col1:
            if st.button("Export Halaman Ini", use_container_width=True, help="Export visualisasi dari halaman yang sedang dibuka"):
                with st.spinner("Mengexport visualisasi..."):
                    zip_bytes = export_current_page()
                    if zip_bytes:
                        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                        current_page = st.session_state.get("navigation", "")
                        page_name = current_page.split(":")[0].strip().replace(" ", "_")
                        st.download_button(
                            label="⬇️ Download ZIP",
                            data=zip_bytes,
                            file_name=f"export_{page_name}_{timestamp}.zip",
                            mime="application/zip",
                            use_container_width=True,
                        )
        
        with col2:
            if st.button("Export Semua", use_container_width=True, help="Export visualisasi dari semua halaman"):
                with st.spinner("Mengexport semua visualisasi..."):
                    zip_bytes = export_all_pages()
                    if zip_bytes:
                        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                        st.download_button(
                            label="⬇️ Download ZIP",
                            data=zip_bytes,
                            file_name=f"export_all_pages_{timestamp}.zip",
                            mime="application/zip",
                            use_container_width=True,
                        )
        
        st.info("💡 Tip: Buka setiap halaman terlebih dahulu, lalu klik 'Export Semua' untuk export lengkap.")
